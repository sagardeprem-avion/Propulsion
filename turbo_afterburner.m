...
afterburner=0 ;              % 0 if afterburner not used, 1 if used
...
prc=linspace(3, 8,20)            %  compressor pressure ratio
...
for i=1:length(prc)
    ...
    f_ab(i)=0;				% initializing the afterburner fuel fraction 
    %  compressor stage
    P03(i)=(P02*prc(i)) 
    ...
    e_prop(i)=2*t(i)*u/(t(i)*u*2+(1+f(i)+f_ab(i))*(v7(i)-u)^2)   % propulsive efficiency
    e_therm(i)=(t(i)*u*2+(1+f(i)+f_ab(i))*(v7(i)-u)^2)/(2*(f(i)+f_ab(i))*Q) %thermal efficiency
...
%hold on
  thrust=t*m 
       plot(prc,t)
        title('Variation of specific thrust with afteburner disabled')
